...
# ─── FAYLLAR ──────────────────────────────────────────────────────────────────
WORDS_FILE       = "rasm_sozlar.json"                              # so'zlar bazasi — repo bilan keladi, o'zgarmaydi
_DATA_DIR        = os.getenv("DATA_DIR", ".").rstrip("/")
pathlib.Path(_DATA_DIR).mkdir(parents=True, exist_ok=True)
DRAW_STATE_FILE  = os.path.join(_DATA_DIR, "drawing_game_states.json")
DRAW_LIKES_FILE  = os.path.join(_DATA_DIR, "drawing_likes.json")
DRAW_SCORES_FILE = os.path.join(_DATA_DIR, "drawing_star_scores.json")

# Redis orqali doimiy zaxiralanadigan fayllar (o'yin holati, layklar, yulduzlar)
_PERSISTED_FILES = [DRAW_STATE_FILE, DRAW_LIKES_FILE, DRAW_SCORES_FILE]

def persisted_files() -> list:
    """main.py startup'da Redis'dan tiklash uchun fayllar ro'yxatini beradi."""
...
def _save(path, data):
    ...
    if path in _PERSISTED_FILES:
        storage.backup_file(path)   # orqa fonda Redis'ga zaxiralash (fire-and-forget)
...
# Telefon klaviaturalari apostrofni turlicha belgi bilan almashtiradi
# (masalan ' o'rniga ' yoki ʻ). Solishtirishdan oldin bularni bitta xil
# belgiga keltiramiz, aks holda "to'g'ri" javob ham noto'g'ri deb hisoblanadi.
...
def _normalize_answer(text: str) -> str:
    ...
    return " ".join(text.split())  # ortiqcha bo'shliqlarni ham tozalash

def _levenshtein(a: str, b: str) -> int:
    """Ikki so'z orasidagi harf farqi (tahrirlash masofasi)."""
    if a == b:
        return 0
    if not a:
        return len(b)
...
async def _react(bot: Bot, chat_id: int, message_id: int, emoji: str):
    """Xabarga emoji reaksiya qo'yadi (xato bo'lsa jim o'tkazib yuboradi)."""
    try:
        ...
        log.warning("reaction error: %s", e)
...
async def check_drawing_answer(message: types.Message, bot: Bot) -> bool:
    ...
    if not state or state.get("status") not in ("waiting", "submitted"):
        return False
    ...
    user_answer  = _normalize_answer(message.text or "")
    correct_word = _normalize_answer(state.get("word", ""))

    if user_answer != correct_word:
        # Yaqin javob tekshiruvi: 1-2 harf farq bo'lsa va uzunligi mos kelsa — 👀
        if user_answer and abs(len(user_answer) - len(correct_word)) <= 2:
            distance = _levenshtein(user_answer, correct_word)
            if 1 <= distance <= 2:
                await _react(bot, chat_id, message.message_id, "👀")
        return False

    # ✅ To'g'ri javobga tabrik reaksiyasi
    await _react(bot, chat_id, message.message_id, "🎉")
    ...
    # Endi haqiqiy message_id bilan Layk tugmasini ham qo'shamiz
    final_kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text          = "👍 Layk",
            callback_data = f"draw_like:{chat_id}:{sent.message_id}:{drawer_id}",
        )],
        [InlineKeyboardButton(
            text          = "🆕 Yangi o'yin boshlash",
            callback_data = f"draw_restart:{chat_id}",
    ...
    try:
        ...
        log.warning("edit result keyboard error: %s", e)
    return True
...
async def _api_draw_word(request):
    ...
    if not user_id or user_id != state.get("drawer_id"):
...
async def _api_draw_reword(request):
    """
    Mini app ichida — chizuvchi so'zni almashtirmoqchi bo'lsa chaqiriladi.
    Faqat hali chizishni boshlamagan (canvas bo'sh yoki yo'q) holatda ishlaydi.
    """
    from fastapi.responses import JSONResponse
    ...
    if state.get("status") != "waiting":
        return JSONResponse({"error": "so'z almashtirib bo'lmaydi"}, status_code=409)
    ...
    state["word"]        = new_word
    state["used_words"]  = used
    state["custom_word"] = False
    _set_state(chat_id, state)

    log.info("Reword: chat=%s new_word=%s", chat_id, new_word)
    return JSONResponse({"ok": True, "word": new_word})

async def _api_draw_restore(request):
    """
    Mini app ochilganda chaqiriladi — agar oldin saqlangan kanvas bo'lsa
    (mini app yopib qayta ochilgan bo'lsa ham), uni qaytaradi.
    """
    from fastapi.responses import JSONResponse
    params     = request.query_params
    session_id = params.get("session")
    ...
    # Faqat chizuvchi o'ziniki tiklay oladi
    user_id = _user_id_from_init_data(init_data)
    if not user_id or user_id != state.get("drawer_id"):
        return JSONResponse({"error": "forbidden"}, status_code=403)

    # Faqat hali "waiting" (chizish jarayonida) bo'lsa tiklanadi
    if state.get("status") != "waiting":
        return JSONResponse({"image_base64": None})

    return JSONResponse({"image_base64": state.get("canvas_snapshot")})
...
async def _api_draw_submit(request):
    ...
    try:
        ...
        if not final_msg_id:
            # Yangi rasm xabari yuborish (tugmalarsiz)
...
async def _api_draw_preview(request):
    ...
    # Kanvasni serverga saqlaymiz — mini app yopib qayta ochilganda tiklash uchun
    state["canvas_snapshot"] = image_b64
    _set_state(chat_id, state)
...
async def setup_fastapi(bot: Bot, app):
    ...
    app.add_route("/api/draw/reword",  _api_draw_reword,  methods=["POST"])
    app.add_route("/api/draw/restore", _api_draw_restore, methods=["GET"])
