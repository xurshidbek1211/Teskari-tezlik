...
API_TOKEN = os.getenv("API_TOKEN")  # Bu sizning bot tokeningiz
RENDER_EXTERNAL_URL = os.getenv("RENDER_EXTERNAL_URL")
WEBHOOK_PATH = f"/webhook/{API_TOKEN}"
WEBHOOK_URL = f"{RENDER_EXTERNAL_URL}{WEBHOOK_PATH}"

ADMIN_ID = 1899194677  # Admin ID
...
def normalize_answer(text):
    return text.lower().replace("ʼ","'").replace("`","'").replace("´","'").replace("‘","'").replace("’","'").strip()
...
async def send_new_question(chat_id):
    ...
    states[str(chat_id)] = {"current": question, "answered_by": None, "chat_id": chat_id}
...
# --- /boshla (har bir foydalanuvchi ishlata oladi) ---
...
# --- /add (faqat ruxsat berilganlar) ---
@dp.message_handler(commands=["add"])
async def add_question(message: types.Message):
    logging.info(f"/add chaqirildi: user_id={message.from_user.id}")
...
async def show_score(message: types.Message):
    ...
    user_score = chat_scores.get(str(message.from_user.id), 0)
...
async def check_answer(message: types.Message):
    try:
        ...
        if chat_id not in states: return
        state = states[chat_id]
        if "current" not in state or state.get("answered_by") is not None: return

        user_answer = normalize_answer(message.text)
        correct_raw = state["current"]["javob"]
        correct_list = [normalize_answer(j) for j in correct_raw] if isinstance(correct_raw, list) else [normalize_answer(correct_raw)]
        ...
        if user_answer in correct_list:
            ...
            for i, (uid, ball) in enumerate(top):
                try:
                    name = (await bot.get_chat(int(uid))).first_name
                except:
                    name = "👤 Nomaʼlum"
                reyting += f"{i+1}. {name} - {ball} ball\n"

            javob_text = "\n".join(correct_raw) if isinstance(correct_raw, list) else correct_raw
            await message.answer(f"🎯 To‘g‘ri javob: {javob_text}\n🎉 {message.from_user.full_name} 1 ball oldi!\n\n🏆 Guruhdagi eng yaxshi 10 ta foydalanuvchi:\n{reyting}")
            await send_new_question(message.chat.id)
    except Exception as e:
        logging.exception("❌ Javobni tekshirishda xato:")
...
async def manual_tabrik(message: types.Message):
    try:
        ...
        try: name = (await bot.get_chat(int(winner_id))).first_name
        except: name = "👤 Nomaʼlum"
        try: chat_title = (await bot.get_chat(int(chat_id))).title
        except: chat_title = "Nomaʼlum guruh"

        congrat_msg = f"""🌟🌸 TABRIKLAYMIZ! 🌸🌟

🥇 Bugungi kunning G‘OLIBI: {name}!
🎉 1-o‘rinni egallaganingiz bilan tabriklaymiz! 🎉
...
🏆 Guruh: {chat_title}"""
        ...
        logging.exception("❌ /tabrik xatosi:")
...
async def show_top_winners(message: types.Message):
    try:
        ...
        for idx, (user_id, count) in enumerate(sorted_winners, start=1):
            try: name = (await bot.get_chat(int(user_id))).first_name
            except: name = "👤 Nomaʼlum"
            text += f"{idx}. {name} — {count} marta\n"

        await message.answer(text, parse_mode="Markdown")
    except Exception as e:
        logging.exception("❌ /kun xatosi:")
...
async def process_webhook(request: Request):
    try:
        ...
        logging.exception("❌ Webhook update xatosi:")
    return {"status": "ok"}
